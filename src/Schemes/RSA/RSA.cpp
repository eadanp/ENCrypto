...
vector<InfInt> generatePandQ() {
    cout << "loading... gonna take a few minutes, stand by\n";
    InfInt p = PrimeNumberGenerator(215);
    InfInt q = PrimeNumberGenerator(215);
    cout << "keys are done generating!\n";
    vector<InfInt> pq;
    pq.push_back(p);
    pq.push_back(q);
    return pq;
...
//publicKey[n, e]
InfInt encryption(InfInt n, InfInt e, InfInt r, string stringMsg) {
 /*   InfInt n = publicKey[0];
    InfInt e = publicKey[1];*/
    InfInt decMsg = binaryToDecimal(stringMsg);
